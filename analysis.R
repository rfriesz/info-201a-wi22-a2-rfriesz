...
setwd("C:/Users/ryanf/Desktop/Data Science 201/a2-starter-rfriesz")
...
national <- read.csv(file = "C:\\Users\\ryanf\\Desktop\\Data Science 201\\us.csv")

# 1.c Load the *state level* data into a variable. `states`
states <- read.csv("C:\\Users\\ryanf\\Desktop\\Data Science 201\\us-states.csv")

# 1.d Load the *county level* data into a variable. `counties`
#     NOTE: This is a large dataset. It may take 30-60 seconds to load.
counties <- read.csv("C:\\Users\\ryanf\\Desktop\\Data Science 201\\us-counties.csv")
...
state_highest_ratio <- subset(states, date==as.Date("2022-02-01"))
state_highest_ratio <- state_highest_ratio %>%
  mutate(death_case_ratio = deaths/cases)%>%
  filter(death_case_ratio == max(death_case_ratio, na.rm=TRUE))%>%
  pull(state)
...
state_lowest_cases <-subset(states, date==as.Date("2022-02-01"))
state_lowest_cases <- state_lowest_cases %>%
...
national <- national %>%
  mutate(lag = lag(national, n = 1L,default = NA, order_by = cases,))%>%
...
national <- national %>%
  mutate(death_lag = lag(national, n = 1L, default = NA, order_by = deaths,))%>%
...
# the data.
ncz <- counties[, c("county", "state", "deaths", "date")] %>%
  filter(date == max(date, na.rm = TRUE))%>%
  filter(deaths == 0)
new_ncz <- ncz %>% 
...
total <- counties[, c("county", "state", "date")] %>%
  filter(date == max(date, na.rm = TRUE))
new_total <- total %>%
  group_by(state) %>% summarise(count = n())


merger <- merge(new_total, new_ncz,by="state", all=T)

merger <- merger %>%
  dplyr::mutate(count.y = replace_na(count.y, 0))
names(merger)[1] <-"state"
names(merger)[2] <-"T"
names(merger)[3] <-"ncz"
prop_no_deaths <- merger %>%
  mutate(prop = ncz/T)
...
all_totals <- merge(national, totals_by_day, by="date") %>%
  rename(national_total_cases = cases)
all_totals$deaths <- NULL
...
national_state_diff <- all_totals %>%
  summarize(national_total_cases != state_total_cases)

national_state_diff <- sum(national_state_diff, na.rm = TRUE)
...
state_county_diff <- all_totals %>%
  summarize(state_total_cases != county_total_cases) 
  
state_county_diff <- sum(state_county_diff, na.rm = TRUE)
...
# QUESTION:  Write your question for 5.a here ... The difference between the county, state, and national number of deaths.
#            ..................................    and the code is ... 
the_answer_is <-  
  
county_by_death <- counties %>%
  group_by(date) %>%
  summarize(county_total_deaths = sum(deaths, na.rm = TRUE))

state_by_death <- states %>%
  group_by(date) %>%
  summarize(state_total_deaths = sum(deaths))

deaths_by_day<- merge(county_by_death,state_by_death,by="date")

death_totals <- merge(national, deaths_by_day, by="date") %>%
  rename(national_total_deaths = deaths)

death_totals$cases <- NULL
